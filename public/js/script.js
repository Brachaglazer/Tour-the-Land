...
function openSampleTripsModal() {
    ...
    modal.innerHTML = `
        <div class="modal-header">
            <h2>Choose a preplanned trip</h2>
            <button type="button" class="modal-close" onclick="closeTripModal()">×</button>
        </div>
        <div class="modal-body">
            <div class="section-title">Pick a sample itinerary and customize it later, while the original stays unchanged.</div>
            ${sampleTrips.map(sample => `
                <article class="sample-card">
                    <div class="sample-card-header">
                        ...
                        <span class="sample-date">${formatTripDate(sample.start_date)} - ${formatTripDate(sample.end_date)}</span>
                    </div>
                    <div class="sample-activities">
                        ${sample.activities.map(activity => `
                            <div class="sample-activity">
                                <span class="sample-activity-time">${activity.time}</span>
                                <div>
                    ...
                    <div class="modal-actions">
                        <button type="button" class="btn" onclick="addSampleTrip('${sample.id}')">Add this trip</button>
                    </div>
                </article>
            `).join('')}
        </div>
...
function addSampleTrip(sampleId) {
    const sample = sampleTrips.find(item => item.id === sampleId);
    if (!sample) return;

    const tripPayload = {
        title: sample.title,
        description: sample.description,
        start_date: sample.start_date ? new Date(sample.start_date).toLocaleDateString() : '',
        end_date: sample.end_date ? new Date(sample.end_date).toLocaleDateString() : '',
        activities: sample.activities.map(activity => ({
            activityId: activity.activityId || `${sample.id}-${Math.random().toString(36).slice(2, 10)}`,
